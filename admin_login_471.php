...
session_start();
	...
	$admin2 = "sumaiya@tabletop.com";
	$admin3 = "redowan@tabletop.com";
	$admin="admin@tabletop.com";
		...
		if(!empty($email) && !empty($password))
		{
			...
			if($result)
			{
				if($result && mysqli_num_rows($result) > 0)
				{
					...
					$a="select email from admin where email='$pemail'";
					$aq = mysqli_query($con, $a);
					$af=mysqli_fetch_array($aq);
					if($user_data['password'] === $password )
						
					{	
						
						
						if($user_data['email'] === $admin2){
							$_SESSION['email'] = $user_data['email'];
							$var= $user_data['email'];
							$q1="insert into temp_auth (email,type) values ('$var','admin')";
							mysqli_query($con,$q1);
							header("Location: admin_homepage_471.php");
							die;
						}
