...
session_start();
	...
	$chef1 = "redowan@chef.com";
	$chef2 = "chef@tabletop.com";
		...
		if(!empty($email) && !empty($password))
		{
			...
			if($result)
			{
				if($result && mysqli_num_rows($result) > 0)
				{
					...
					$c="select email from chef where email='$pemail'";
					$cq = mysqli_query($con, $c);
					$cf=mysqli_fetch_array($cq);
					
					if($user_data['password'] === $password )
						
					{	
						if ($user_data['email'] === $chef1){
							$_SESSION['email'] = $user_data['email'];
							$var= $user_data['email'];
							$q1="insert into temp_auth (email,type) values ('$var','chef')";
							mysqli_query($con,$q1);
							header("Location: chef_homepage_471.php");
							die;

						}
						elseif($user_data['email'] === $chef2){
							$_SESSION['email'] = $user_data['email'];
							$var= $user_data['email'];
							$q1="insert into temp_auth (email,type) values ('$var','chef')";
							mysqli_query($con,$q1);
